...
\usepackage{amsmath, tikz, xcolor, pgfplots, array}
...
\begin{axis}[
    ...
    \addplot [blue, domain = -1.5:1.75,samples=300, <->, very thick]
      {x^2 - 0.5};
    \plot [fill = black] (1,1.5) circle (3pt);
    \plot [blue, fill = white] (1,0.5) circle (3pt);
\end{axis}
\end{tikzpicture}
\end{minipage} 
\hspace{0.45cm}
...
\begin{enumerate}
    \item multiply by 3 \quad \onslide<2->{$3x$}    \newline\\
    \item add 4 \quad \onslide<3->{$3x+4$}  \newline\\
\end{enumerate}
\onslide<4->{\[f(x) = 3x + 4\]}
\end{frame}
...
\begin{enumerate}
    \item add 4 \quad \onslide<2->{$x+4$}   \newline\\
    \item multiply by 3 \quad \onslide<3->{$3(x+4)$}    \newline\\
\end{enumerate}
\onslide<4->{\[g(x) = 3x + 12\]}
\end{frame}
